...
class Bot:
    ...
    def _trade_live(self, bid: float, ask: float) -> None:
        ...
        # simple buy/sell near the edges
        buy_price = bid
        sell_price = ask

        # requote logic using local open_order state
        if self.open_order:
        ...
        # if we have base inventory, prefer SELL; otherwise BUY
        if base_bal >= max(self.cfg.min_base_lot, base_qty_for_quote * 0.9):
            ...
            self._set_open_order({"ts": time.time(), "side": "SELL", "price": sell_price, "qty": base_qty})
    ...
    def _trade_sim(self, bid: float, ask: float) -> None:
        ...
        if self.sim_quote >= self.cfg.trade_quote_amount:
            ...
            self.db.add_trade(now_utc(), "BUY", ask, base_qty, self.cfg.trade_quote_amount, 0.0, "SIM")
            self._set_last_trade_ts(time.time())
            ...
            self.db.add_trade(now_utc(), "SELL", bid, base_qty, quote_got, 0.0, "SIM")
            self._set_last_trade_ts(time.time())
